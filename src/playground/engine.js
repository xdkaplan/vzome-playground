...
//
//   - vzomeEngine (TODO): the real golden-field engine, loaded from the vZome
...
// Vec:  { plus(Vec): Vec, times(n): Vec, isZero(): boolean, key(): string }
    ...
    catalog() {
      ...
      return {
        add(axes) {
          ...
          const k = axes.map((a) => a.vector().key()).sort().join('|');
...
export async function createEngine() {
  // TODO: when the vZome engine bundle is available, detect/load it here and
  // return vzomeEngine(); fall back to toyEngine() otherwise.
  return toyEngine();
}
